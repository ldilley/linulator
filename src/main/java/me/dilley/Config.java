...
public class Config
{
  ...
  public boolean getReadonlyDatabase()
  {
    return readonlyDatabase;
  }

  public void setReadonlyDatabase(String readonlyDatabase)
  {
    try
    {
      if(readonlyDatabase.length() > 0)
      {
        if(readonlyDatabase.equals("1") || readonlyDatabase.equalsIgnoreCase("on") || readonlyDatabase.equalsIgnoreCase("enabled"))
          this.readonlyDatabase = true;
        else
          this.readonlyDatabase = Boolean.parseBoolean(readonlyDatabase);
      }
      else
      {
        Log.write(1, "readonly_database value is not set. Setting read-only database mode to: Enabled");
        System.out.println("Warning: readonly_database value is not set. Setting read-only database mode to: Enabled");
        this.debugMode = Defaults.DEFAULT_READONLY_DATABASE;
      }
    }
    catch(NumberFormatException nfe)
    {
      Log.write(1, "readonly_database value is invalid. Setting read-only database mode to: Enabled");
      System.out.println("Warning: readonly_database value is invalid. Setting read-only database mode to: Enabled");
      this.readonlyDatabase = Defaults.DEFAULT_READONLY_DATABASE;
    }
  }
  ...
  public void parseConfig()
  {
    ...
    try
    {
      ...
      setReadonlyDatabase(validateOption(config, "readonly_database"));
  ...
  public void showConfig()
  {
    ...
    System.out.println("Read-only Database: " + getReadonlyDatabase());
